...
const getDiasDisponiveis = async (medicoId) => {
  try {
    ...
    const datasOcupadas = new Set(
      result.rows.map((row) => row.data_consulta.toISOString().split("T")[0])
    );
...
// Função para buscar consultas de um paciente pelo ID
const getConsultasByPacienteId = async (pacienteId) => {
  try {
    const result = await pool.query(
      `
            SELECT 
...
// Função para cancelar consulta pelo ID
...
// Função para reagendar consulta com nova data
const reagendarConsulta = async (consultaId, novaData) => {
  try {
    await pool.query(
      `
            UPDATE consultas
