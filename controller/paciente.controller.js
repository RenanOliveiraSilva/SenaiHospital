...
const getHorariosDisponiveis = async (req, res) => {
  try {
    ...
    if (!medicoId || !dataConsulta) {
      return res
        .status(400)
        .json({ message: "Médico e data são obrigatórios" });
    }

    const horariosDisponiveis = await PacienteModel.getHorariosDisponiveis(
      medicoId,
      dataConsulta
    );
...
const agendarConsulta = async (req, res) => {
  try {
    ...
    const result = await PacienteModel.agendarConsulta({
      data,
      horario,
      medicoId: medicoIdFormatado,
      localConsulta,
      pacienteId,
    });
...
const renderizarCancelarConsulta = async (req, res) => {
  try {
    const pacienteId = req.query.pacienteId; // ID do paciente via query string

    if (!pacienteId) {
      return res
        .status(404)
        .render("error", { message: "Paciente não encontrado", error: {} });
    }

    // Aqui você pode adicionar lógica para carregar dados relacionados se necessário

    res.render("./homeUsuario/cancelar-consulta", { pacienteId });
  } catch (error) {
    console.error(
      "Erro ao renderizar a página de cancelamento de consultas:",
      error
    );
    res
      .status(500)
      .render("error", { message: "Erro ao carregar a página", error });
  }
};
...
const buscarConsultas = async (req, res) => {
  try {
    ...
    if (!pacienteId) {
      return res.status(400).send("Paciente ID não fornecido.");
    }

    const consultas = await pacienteModel.getConsultasByPacienteId(pacienteId);

    if (!consultas || consultas.length === 0) {
      return res.render("./homeUsuario/visualizar-cancelar-consultas", {
        consultas: [],
        pacienteId,
    ...
    res.render("./homeUsuario/visualizar-cancelar-consultas", {
      consultas,
      pacienteId,
    });
  } catch (error) {
...
const buscarHorariosDisponiveis = async (req, res) => {
  try {
    const { medicoId, dataConsulta } = req.query;
    ...
    if (!medicoId || !dataConsulta) {
      return res
        .status(400)
        .send("Dados incompletos para buscar horários disponíveis.");
    }

    const horarios = await pacienteModel.getHorariosDisponiveis(
      medicoId,
      dataConsulta
    );
...
const cancelarConsulta = async (req, res) => {
  try {
    const { consultaId, motivo, pacienteId } = req.body;
    if (!consultaId || !motivo) {
      return res.status(400).send("Dados incompletos para cancelar consulta.");
    }

    await pacienteModel.cancelarConsulta(consultaId, motivo);
    res.redirect(`/consultas?pacienteId=${pacienteId}`);
  } catch (error) {
...
// Reagendar consulta
const reagendarConsulta = async (req, res) => {
  try {
    const { consultaId, novaData, pacienteId } = req.body;
    if (!consultaId || !novaData) {
      return res.status(400).send("Dados incompletos para reagendar consulta.");
    }

    await pacienteModel.reagendarConsulta(consultaId, novaData);
    res.redirect(`/consultas?pacienteId=${pacienteId}`);
  } catch (error) {
    console.error("Erro ao reagendar consulta:", error);
    res.status(500).send("Erro ao reagendar consulta.");
  }
};
